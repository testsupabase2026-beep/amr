...
# ── Embeddings backend ────────────────────────────────────────
# Gemini API embeddings (zero local RAM — ideal for free-tier hosts) when
# GOOGLE_API_KEY is set; otherwise the local multilingual MiniLM model.
# The FAISS index MUST be built with the SAME backend/model/dim it is queried
# with — rebuild via scripts/build_dwh_index.py after changing these.
GEMINI_EMBED_MODEL    = "models/gemini-embedding-001"   # multilingual (Arabic+English)
GEMINI_EMBED_DIM      = 768                             # output_dimensionality (lean, high quality)
LOCAL_EMBED_MODEL     = "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"  # 384-dim
        ...
        if self._impl is None:
            with self._lock:
                if self._impl is None:
                    ...
                    if google_key:
                        # Gemini API embeddings — model runs on Google's servers
                        # (zero local RAM). Multilingual (Arabic+English). Same
                        # model+dim as the index = FAISS stays compatible.
                        ...
                        print(
                            f"✅ Embeddings ready (Gemini API "
                            f"{GEMINI_EMBED_MODEL}, {GEMINI_EMBED_DIM}d — zero local RAM)."
                        )
                    else:
                        # No key → local multilingual MiniLM model (~300MB RAM).
                        self._impl = HuggingFaceEmbeddings(
                            model_name=LOCAL_EMBED_MODEL,
                            model_kwargs={"device": "cpu"},
                        )
                        print("✅ Embeddings ready (local MiniLM model).")
